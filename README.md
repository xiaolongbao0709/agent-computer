...
- **一键同步（推荐）**：你的仓库页 → **Actions** → 「同步上游模板」→ **Run workflow**。
  它会把模板最新文件搬进你的仓库并提交，Cloudflare 随即自动重新部署（每周一也会自动跑一次；
  不想要自动跑，删掉工作流里的 `schedule` 段即可）。你的 `wrangler.jsonc`（容器/isolate 开关）
  不会被覆盖。Worker 地址、密钥、硬盘数据在更新中全部保持不变。
- **手动**：把模板仓库里变化的文件复制进你的仓库提交即可，效果相同。

（如果你的仓库是在本工作流加入之前部署的，Actions 里没有这个按钮：手动把模板的
`.github/workflows/sync-template.yml` 复制进你的仓库一次，以后就有了。）
